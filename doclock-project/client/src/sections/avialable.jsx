...
export default function Avialable() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return doctors;
    return doctors.filter((d) => `${d.name} ${d.tag} ${d.city}`.toLowerCase().includes(q));
  }, [doctors, query]);
              ...
              <div className="avail-list">
                {filtered.map((d, idx) => (
                  <article key={d.id} className="avail-card">
                    ...
                    <button className="avail-book" type="button" onClick={() => navigate('/set-appointment')}>
                      Book Now
                    </button>
                  </article>
                ))}
