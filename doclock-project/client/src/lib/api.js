...
const DICEBEAR = 'https://api.dicebear.com/7.x/initials/svg'

function dicebearUrl(seed) {
  return `${DICEBEAR}?seed=${encodeURIComponent(seed)}&backgroundType=gradientLinear`
}

/** Uploaded `user.avatar`, or a stable DiceBear image from id/email/name. */
export function getProfileImageSrc(user) {
  if (!user) return null
  const a = user.avatar
  if (typeof a === 'string' && a.trim().length > 0) return a.trim()
  const seed = String(user.email || user.name || user._id || user.id || 'user')
  return dicebearUrl(seed)
}

/** Doctor row avatar: populated `doctor`, else parsed from `service`, else fallback. */
export function getAppointmentDoctorImageSrc(appt) {
  if (!appt || typeof appt !== 'object') {
    return dicebearUrl('appointment')
  }
  const d = appt.doctor
  if (d && typeof d === 'object') {
    const a = d.avatar
    if (typeof a === 'string' && a.trim().length > 0) return a.trim()
    const seed = String(d._id || d.id || d.name || 'doctor')
    return dicebearUrl(seed)
  }
  const { doctor } = specialtyAndDoctorFromService(appt.service)
  const seed = String(doctor || appt._id || appt.id || 'appointment')
  return dicebearUrl(seed)
}
...
/** Prefer `getProfileImageSrc(useAuth().user)`. */
